    ...
    def run(self, request: GenerateSkillRequest) -> tuple[str, list[GeneratedFile]]:
        ...
        skill_md_task = Task(
            description=(
                ...
                "ACTUAL steps/patterns (use their words, not generic labels)\n"
                "- 'Hard Constraints' section: list the creator's exact restrictions, quoting their language\n"
                "- 'Output Format' section: show the exact output structure the skill produces\n"
                ...
                "- Use the creator's actual phrases in the constraint list\n"
                "- Do NOT use any language from the varun-maya example"
            ),
            expected_output="A concise SKILL.md (under 60 lines) with frontmatter, framework overview using creator's language, constraints, output format, and references.",
            agent=writer,
        ...
        examples_task = Task(
            description=(
                ...
                "For instance, if the creator's skill is about repurposing YouTube videos into "
                "short-form content, then:\n"
                "- The INPUT for each example = a hypothetical YouTube video summary/transcript\n"
                "- The OUTPUT = the actual repurposed content the skill would produce\n"
                "- The examples must show ALL output types the creator specifies\n\n"
